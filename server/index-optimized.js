...
function makeDocumentPayload(message, media) {
  ...
  const fileName = media.filename || `${message.id?._serialized || randomUUID()}.bin`;
  const category = categoryFromMime(media.mimetype || "", fileName);
  const folder = inboxDirs[category] || inboxDirs.others;
  const extension = inferExtension(media.mimetype || "", fileName);
  const baseName = `${new Date().toISOString().replace(/[:.]/g, "-")}_${safeName(message.id?._serialized || randomUUID())}_${safeName(path.basename(fileName, path.extname(fileName)) || "incoming")}${extension}`;
...
app.post("/api/queue", (request, response) => {
  ...
  if (!documentId || !filePath || !fileName) {
    response.status(400).json({ error: "documentId, fileName, and filePath are required" });
    return;
  }
  ...
  response.json({ job, queue: state.queue });
});
